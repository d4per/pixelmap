...
impl Model3D {
    ...
    ///    multiplies by an assumed depth ratio (0.3) times the smaller image-plane extent.
    ///
    /// # Returns
    ...
    /// - If any pair of points is too far apart, that face is skipped. Depth counts as
    ///   much as the image plane here, even though the model compresses it.
...
mod tests {
    ...
    #[test]
    fn depth_spans_z_depth_ratio_of_the_smaller_extent() {
        let model = Model3D::new(&mapping(|px, py| 8.0 * (1.0 - radius(px, py)).max(0.0)));
